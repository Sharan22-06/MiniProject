...
// Context to propagate a default drawer direction from the Root to Content
type DrawerDirection = 'top' | 'bottom' | 'left' | 'right'
const DrawerDirectionContext = React.createContext<DrawerDirection | undefined>(undefined)
...
function Drawer({
  ...
  return (
    <DialogPrimitive.Root data-slot="drawer" {...props}>
      <DrawerDirectionContext.Provider value={defaultDirection}>
        {children}
...
function DrawerContent({
  className,
  children,
  // allow callers to opt-in to a drawer direction (top, bottom, left, right)
  direction,
  ...props
}: React.ComponentProps<typeof DialogPrimitive.Content> & {
  direction?: DrawerDirection
}) {
  const ctxDirection = React.useContext(DrawerDirectionContext)
  const resolvedDirection = direction ?? ctxDirection ?? 'right'
  return (
    <DrawerPortal data-slot="drawer-portal">
      <DrawerOverlay />
      <DialogPrimitive.Content
        data-slot="drawer-content"
        data-drawer-direction={resolvedDirection}
        className={cn(
